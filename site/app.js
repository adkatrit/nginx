(() => {
  ...
  const sunBackgroundCanvas = /** @type {HTMLCanvasElement|null} */ ($("sunBackgroundCanvas"));
  ...
  // Sun settings panel elements
  const sunSettingsPanel = $("sunSettingsPanel");
  const sunEnabledToggle = /** @type {HTMLInputElement|null} */ ($("sunEnabledToggle"));
  const sunXSlider = /** @type {HTMLInputElement|null} */ ($("sunXSlider"));
  const sunYSlider = /** @type {HTMLInputElement|null} */ ($("sunYSlider"));
  const sunSizeSlider = /** @type {HTMLInputElement|null} */ ($("sunSizeSlider"));
  const sunColorInput = /** @type {HTMLInputElement|null} */ ($("sunColorInput"));
  const sunPulseSlider = /** @type {HTMLInputElement|null} */ ($("sunPulseSlider"));
  const sunGlowSlider = /** @type {HTMLInputElement|null} */ ($("sunGlowSlider"));
  const closeSunSettingsBtn = $("closeSunSettings");
  ...
  // ---- Three.js visualizer ----
  // WebGPU renderer with TSL post-processing (forceWebGL keeps GLSL ShaderMaterials working)
  const THREE_CDN = "three";
  const ORBIT_CONTROLS_CDN = "three/addons/controls/OrbitControls.js";
  const GLTF_LOADER_CDN = "three/addons/loaders/GLTFLoader.js";
  /** @type {any | null} */
  let three = null;
  ...
  // Audio-reactive post-processing uniforms (set after pipeline creation)
  let ppBloomStrength = null;
  let ppBloomThreshold = null;
  let ppBloomRadius = null;
  let ppVignetteIntensity = null;
  let ppChromaticStrength = null;
  ...
  // Track-specific sun configurations (scaled for thin waveform - 36px height)
  const TRACK_SUN_CONFIG = {
    "Data Tide": { x: 0.5, y: 0.5, size: 20, color: '#FF6B35', pulse: 0.4, glow: 0.7, animation: 'sinking' },
    "Signal Integrity": { x: 0.75, y: 0.3, size: 18, color: '#00D9FF', pulse: 0.2, glow: 0.5, animation: 'rising' },
    "Terms & Conditions": { x: 0.5, y: 0.7, size: 16, color: '#FFB700', pulse: 0.25, glow: 0.6, animation: 'setting' },
    "Soft Systems": { x: 0.5, y: 0.4, size: 22, color: '#FFD700', pulse: 0.35, glow: 0.65, animation: 'floating' },
    "Beast Mode": { x: 0.8, y: 0.3, size: 24, color: '#FF3333', pulse: 0.6, glow: 0.8, animation: 'pulsing' },
    "Dreams Bleed Into Dashboards": { x: 0.3, y: 0.5, size: 20, color: '#FF1493', pulse: 0.45, glow: 0.7, animation: 'drifting' },
    "Gi Mi Di Reins": { x: 0.5, y: 0.5, size: 21, color: '#FFA500', pulse: 0.3, glow: 0.6, animation: 'slow' },
    "Trade You My Hands": { x: 0.6, y: 0.4, size: 18, color: '#FFD700', pulse: 0.25, glow: 0.5, animation: 'steady' },
    "Push Harder": { x: 0.2, y: 0.3, size: 23, color: '#FF4500', pulse: 0.55, glow: 0.75, animation: 'intense' },
    "The Last Dragon": { x: 0.5, y: 0.35, size: 25, color: '#FF6347', pulse: 0.5, glow: 0.8, animation: 'rising' },
    "Who's Learning Who": { x: 0.7, y: 0.5, size: 19, color: '#FFD700', pulse: 0.3, glow: 0.6, animation: 'floating' },
    "Turn Your Phone Face Down": { x: 0.5, y: 0.6, size: 20, color: '#FF8C00', pulse: 0.35, glow: 0.65, animation: 'setting' },
    "Test": { x: 0.5, y: 0.5, size: 20, color: '#FFD700', pulse: 0.3, glow: 0.6, animation: 'floating' }
  };
  ...
  class WaveformVisualizer {
    ...
    _drawTrackSun(ctx, width, height, audioData, config, time) {
      if (!this._sunDrawLogged) {
        console.log('SUN DRAWING:', { width, height, config: JSON.stringify(config), time: time?.toFixed(1) });
        this._sunDrawLogged = true;
      }
      // Calculate animation offset
      let yOffset = 0;
      if (config.animation === 'rising') {
        yOffset = Math.sin(time * 0.5) * 2;
      } else if (config.animation === 'sinking' || config.animation === 'setting') {
        yOffset = Math.cos(time * 0.3) * 1;
      } else if (config.animation === 'drifting') {
        yOffset = Math.sin(time * 0.4) * 2;
      } else if (config.animation === 'floating') {
        yOffset = Math.sin(time * 0.2) * 1;
      } else if (config.animation === 'intense') {
        yOffset = Math.sin(time * 1.2) * 2;
      }

      const x = config.x * width;
      const y = (config.y * height) + yOffset;

      // Audio-driven pulse
      let pulse = 1;
      if (audioData && audioData.frequencyData) {
        const avg = Array.from(audioData.frequencyData).reduce((a, b) => a + b, 0) / audioData.frequencyData.length / 255;
        pulse = 1 + (avg * config.pulse);
      }
      if (config.animation === 'pulsing') {
        pulse *= (1 + Math.sin(time * 2) * 0.2);
      }
      ...
      for (let i = 3; i >= 1; i--) {
        ...
        const glowAlpha = (config.glow * 0.15) / i;
        ctx.fillStyle = this._hexToRgba(config.color, glowAlpha);
        ctx.beginPath();
      ...
      gradient.addColorStop(0, this._hexToRgba(config.color, 0.9));
      gradient.addColorStop(0.7, this._hexToRgba(config.color, 0.6));
      gradient.addColorStop(1, this._hexToRgba(config.color, 0.1));
      ...
      // Bright core
      ctx.fillStyle = this._hexToRgba(config.color, 1);
      ctx.beginPath();
  ...
  // Sun background renderer
  class SunBackgroundRenderer {
    ...
    setupResize() {
      if (!this.canvas.parentElement) {
        console.error("Sun canvas has no parent element");
        return;
      }
      this.resizeObserver = new ResizeObserver(() => this.resize());
      this.resizeObserver.observe(this.canvas.parentElement);
      this.resize();
      console.log("Sun canvas resized to:", this.displayWidth, "x", this.displayHeight);
    }
    ...
    draw(audioData) {
      const { ctx, displayWidth: w, displayHeight: h, config } = this;
      if (!w || !h) return;

      ctx.clearRect(0, 0, w, h);

      // Calculate animation offset based on animation type (scaled for 36px height)
      let yOffset = 0;
      if (config.animation === 'rising') {
        yOffset = Math.sin(this.time * 0.5) * 2;
      } else if (config.animation === 'sinking' || config.animation === 'setting') {
        yOffset = Math.cos(this.time * 0.3) * 1;
      } else if (config.animation === 'drifting') {
        yOffset = Math.sin(this.time * 0.4) * 2;
        config.x += Math.cos(this.time * 0.2) * 0.0005;
      } else if (config.animation === 'floating') {
        yOffset = Math.sin(this.time * 0.2) * 1;
      } else if (config.animation === 'pulsing') {
        // Extra pulse handled in size calculation
      } else if (config.animation === 'intense') {
        yOffset = Math.sin(this.time * 1.2) * 2;
      }

      const x = config.x * w;
      const y = (config.y * h) + yOffset;
      ...
      // Extra animation-specific pulsing
      if (config.animation === 'pulsing') {
        pulse *= (1 + Math.sin(this.time * 2) * 0.2);
      }
  ...
  function setSunForTrack(trackTitle) {
    const config = TRACK_SUN_CONFIG[trackTitle];
    if (!sunBackgroundCanvas) {
      console.warn("Sun background canvas not found");
      return;
    }

    if (!config) {
      console.warn("No sun config for track:", trackTitle);
      return;
    }

    // Destroy old renderer
    if (sunRenderer) sunRenderer.destroy();

    // Create new renderer with track config
    const trackConfig = JSON.parse(JSON.stringify(config)); // Deep clone
    sunRenderer = new SunBackgroundRenderer(sunBackgroundCanvas, trackConfig);
    console.log("Sun renderer initialized for:", trackTitle);
  }
  ...
  function updateTimeUi() {
    ...
    if (!isSeeking && Number.isFinite(dur) && dur > 0) {
      ...
      // Update sun time (config is read by waveform draw, no separate canvas render)
      if (!sunRenderer && currentTrackTitle) {
        setSunForTrack(currentTrackTitle);
      }
      if (sunRenderer) {
        sunRenderer.time = Date.now() / 1000;
      }
    }
      ...
      textToCopy = `// Viz params
{
  ...
  function syncThreeVizMode() {
    ...
    // Hide score HUD in free-rotation mode (no gates/scoring)
    if (scoreHud) {
      const envInstance = EnvironmentMode?.instance;
      const freeRot = envInstance && envInstance.freeRotationActive;
      scoreHud.style.display = (isEnvironment && !freeRot) ? 'flex' : 'none';
    }
  ...
  // Track which bg scene is currently wired into the pipeline
  let ppCurrentBgScene = null;

  /**
   * Build (or rebuild) the PostProcessing pipeline.
   * Called once at init and again whenever the background scene changes.
   */
  function buildPostProcessingPipeline(bgScene, bgCamera) {
    if (!threeRenderer || !threeScene || !threeCamera || !three?._tsl || !three?._PostProcessing) return;

    const { pass, Fn, float, vec2, vec4, screenUV, bloom, fxaa, chromaticAberrationPass } = three._tsl;
    const PostProc = three._PostProcessing;

    postProcessing = new PostProc(threeRenderer);

    // Single scene pass — animated background is rendered in the scene itself
    // (added directly to threeScene at renderOrder -1000)
    const scenePass = pass(threeScene, threeCamera);
    let composited = scenePass.getTextureNode("output");
    ppCurrentBgScene = bgScene || null; // Track for rebuild detection

    // Bloom
    const bloomPass = bloom(composited);
    bloomPass.threshold = ppBloomThreshold;
    bloomPass.strength = ppBloomStrength;
    bloomPass.radius = ppBloomRadius;
    let output = composited.add(bloomPass);

    // Vignette (custom TSL)
    const vignetteEffect = Fn(() => {
      const uv = screenUV;
      const dist = uv.sub(0.5).length();
      const vig = float(1.0).sub(dist.mul(ppVignetteIntensity.mul(3.0))).clamp(0, 1);
      return output.mul(vig);
    })();
    output = vignetteEffect;

    // Chromatic aberration (built-in Three.js TSL node)
    output = chromaticAberrationPass(output, ppChromaticStrength);
  ...
  function drawVizThree(nowMs) {
    ...
    if (postProcessing) {
      // Check if we need to rebuild the pipeline with a background scene
      if (bgVizMode === "environment" && EnvironmentMode && EnvironmentMode.instance) {
        const bg = EnvironmentMode.instance.animatedBackground;
        if (bg && bg.bgScene && bg.bgScene !== ppCurrentBgScene) {
          buildPostProcessingPipeline(bg.bgScene, bg.bgCamera);
        }
      } else if (ppCurrentBgScene !== null) {
        // Environment mode off — rebuild without bg
        buildPostProcessingPipeline(null, null);
      }

      postProcessing.render();
    } else {
      // Fallback: direct rendering (no post-processing)
    ...
    threeInitPromise = (async () => {
      try {
        ...
        threeRenderer = new WebGPURenderer({
          ...
          antialias: true,
          alpha: true,
          forceWebGL: !navigator.gpu,  // Native WebGPU when available, WebGL fallback otherwise
        });
        ...
        try {
          ...
          const {
            ...
            length: tslLength, dot: tslDot, cross: tslCross, normalize: tslNormalize,
            atan2: tslAtan2, exp: tslExp, log: tslLog,
            max: tslMax, min: tslMin, sign: tslSign,
            select, instanceIndex, storage, compute,
            mx_noise_float, mx_fractal_noise_float,
            fwidth: tslFwidth, sqrt: tslSqrt, negate,
          ...
          const { chromaticAberration: chromaticAberrationPass } = await import("three/addons/tsl/display/ChromaticAberrationNode.js");

          // Expose TSL functions globally for non-module scripts (environments.js, backgrounds.js, etc.)
          window._TSL = {
            ...
            cameraPosition: tslCameraPosition,
            int: tslInt,
            // Also expose node material classes from Three.js module
            MeshBasicNodeMaterial: mod.MeshBasicNodeMaterial,
            MeshStandardNodeMaterial: mod.MeshStandardNodeMaterial,
            SpriteNodeMaterial: mod.SpriteNodeMaterial,
            PointsNodeMaterial: mod.PointsNodeMaterial,
            StorageBufferAttribute: mod.StorageBufferAttribute,
          };

          // Audio-reactive uniforms
          ppBloomThreshold = uniform(0.6);
          ppBloomStrength = uniform(0.8);
          ppBloomRadius = uniform(0.4);
          ppVignetteIntensity = uniform(0.5);
          ppChromaticStrength = uniform(0.002);

          // Store TSL imports for lazy pipeline rebuild
          three._tsl = { pass, uniform, Fn, float: tslFloat, vec2: tslVec2, vec4: tslVec4, screenUV, bloom, fxaa, viewportSharedTexture, chromaticAberrationPass };
          three._PostProcessing = PostProcessing;

          buildPostProcessingPipeline();
          console.log("Post-processing pipeline created");
        } catch (ppErr) {
          console.warn("Post-processing setup failed, falling back to direct rendering:", ppErr);
          postProcessing = null;
        }
  ...
  // ---- Sun Settings Functions ----

  let sunSettingsVisible = false;

  function toggleSunSettingsPanel() {
    if (!sunSettingsPanel) return;
    sunSettingsVisible = !sunSettingsVisible;
    sunSettingsPanel.hidden = !sunSettingsVisible;
  }

  function setupSunSettings() {
    if (!sunEnabledToggle) return;

    sunXSlider?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.x = parseInt(e.target.value) / 100;
      }
      const val = $("sunXValue");
    ...
    sunYSlider?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.y = parseInt(e.target.value) / 100;
      }
      const val = $("sunYValue");
      if (val) val.textContent = e.target.value;
    });
    ...
    sunSizeSlider?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.size = parseInt(e.target.value);
      }
      const val = $("sunSizeValue");
      if (val) val.textContent = e.target.value;
    });

    sunColorInput?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.color = e.target.value;
      }
    });

    sunPulseSlider?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.pulse = parseInt(e.target.value) / 100;
      }
      const val = $("sunPulseValue");
      if (val) val.textContent = e.target.value;
    });

    sunGlowSlider?.addEventListener("input", (e) => {
      if (sunRenderer && sunRenderer.config) {
        sunRenderer.config.glow = parseInt(e.target.value) / 100;
      }
      const val = $("sunGlowValue");
      if (val) val.textContent = e.target.value;
    });

    closeSunSettingsBtn?.addEventListener("click", () => {
      sunSettingsVisible = false;
      sunSettingsPanel.hidden = true;
    });
  }
